...
nm_k.new <- group_levels.rename(nm_k)
till_k.new <- group_levels.rename(till_k)
unique(cc_k.new$group_level3)
unique(nm_k.new$group_level3)
unique(till_k.new$group_level3)
...
#then rerun website preparation removal code so that only the rows we want are included in the website file
